...
export default class UI{
    ...
    MybooksListDisplay(){       
		...
		books.forEach((book) => this.AddBookToList(book,lienDiv));
		content.append(lienDiv);
       
        
	}
	AddBookToList(id,content){		
        this.ListBook(id).then((result) =>{
    ...
    SearchBook(titre,auteur){
        if(titre === ''  || auteur === ''){
            this.showAlert('le titre et l\'auteur doivent être renseignés !');
        }else{
   ...
   this.SearchBook(titleUi,authorUi).then((result) =>{ 
        var placeHldr = "/images/unavailable.png";
        
        for (var i = 0;i<result.length;i++){
            const item = result[i]; 
            const titre =item.volumeInfo.title.substring(0,10) + "...";
            const auteur = (item.volumeInfo.authors) ? item.volumeInfo.authors[0] : "nom de l'auteur manquant !";
            const identifiant = item.volumeInfo.industryIdentifiers[0].identifier;
            const descript = (item.volumeInfo.description)?item.volumeInfo.description.substring(0,200)+"..." : "description manquante !";
            const imgbook = (item.volumeInfo.imageLinks) ? item.volumeInfo.imageLinks.thumbnail : placeHldr ;
    ...
    AddRemoveBook(){
        ...
        elem.addEventListener("click",(e)=> {
            ...
            const idbook = elem.getAttribute('id');
                if(this.checkExist(storages, idbook) === false) {
                    addBook(idbook);
                    this.AddBookToList(idbook,content);    
                    this.showAlert('Ajout avec succès')  ;            
                }else{
                    removeBook(idbook);  
                    this.deleteBookDisplayBySearch(idbook);              
                    this.showAlert('Suppression avec succès');                   
                }            
            });
        });  
